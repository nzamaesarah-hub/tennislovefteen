...
<body class="bg-slate-100 text-slate-800 antialiased font-sans min-h-screen flex flex-col">
    ...
    <nav class="bg-[#1D2A44] text-white shadow-md">
        <div class="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
            ...
            <a href="{{ url('/') }}" class="text-slate-300 hover:text-white font-medium text-sm flex items-center gap-1">
                ← Kembali ke Beranda
            </a>
        </div>
    </nav>
    ...
    <main class="max-w-3xl mx-auto px-6 py-12 flex-grow w-full">
        <div class="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
            <div class="bg-[#2B5B84] text-white p-6 md:p-8">
                <span class="text-xs font-bold uppercase tracking-widest bg-[#CCFF00] text-[#1D2A44] px-3 py-1 rounded-md">
                    Form Pemesanan
                </span>
                <h1 class="text-2xl md:text-3xl font-black mt-3">{{ $court->name }}</h1>
                <p class="text-slate-200 text-sm mt-1">Tarif: <strong class="text-[#CCFF00]">Rp {{ number_format($court->price_per_hour, 0, ',', '.') }} / jam</strong></p>
            </div>
            ...
            <form action="{{ route('booking.store') }}" method="POST" class="p-6 md:p-8 space-y-6">
                ...
                <div>
                    <label for="booking_date" class="block text-sm font-bold text-[#1D2A44] mb-2">Tanggal Main</label>
                    <input type="date" id="booking_date" name="booking_date" min="{{ date('Y-m-d') }}" value="{{ old('booking_date', date('Y-m-d')) }}" required
                        class="w-full rounded-xl border-slate-300 focus:border-[#2B5B84] focus:ring-[#2B5B84] p-3 text-slate-700 shadow-sm">
                </div>

                <div>
                    <label for="start_time" class="block text-sm font-bold text-[#1D2A44] mb-2">Jam Mulai</label>
                ...
                <div>
                    <label for="duration_hours" class="block text-sm font-bold text-[#1D2A44] mb-2">Durasi (Jam)</label>
                ...
                <button type="submit" class="w-full bg-[#CCFF00] hover:bg-[#bce600] text-[#1D2A44] font-extrabold py-4 rounded-xl transition shadow-lg text-base">
